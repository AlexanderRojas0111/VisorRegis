        ...
        <table class="table" id="qrcodes-table">
            ...
            @foreach($qrcodes as $qrcode)
                <tr>
                    ...
                    <td><a href="{{ asset($qrcode->webside) }}">$qrcode->webside</a></td>
                    
                    <td>{{ $qrcode->company_name }}</td>
                    <td>{{ $qrcode->product_name }}</td>
                    <td>{{ $qrcode->product_url }}</td>
                    <td><img src="{{ asset($qrcode->product_urlImagePath) }}" width="30%"  ></td>
                    <td>{{ $qrcode->callback_url }}</td>
                    <td><img src="{{ asset($qrcode->qrcode_path) }}" width="40%"></td>
                    <td>{{ $qrcode->amount }}</td>
